...
passport.use(new GoogleStrategy({
        ...
        try {
            ...
            if (!existingUser) {
                const newUser = await new User({ googleID: profile.id, name: profile.displayName, email: profile.emails[0].value }).save();
                done(null, newUser);
            }
...
passport.use(new FacebookStrategy({
    ...
    profileFields: ['id', 'displayName', 'email']
        ...
        try {
            ...
            if (!existingUser) {
                const newUser = await new User({ facebookID: profile.id, name: profile.displayName, email: profile.emails[0].value }).save();
                done(null, newUser);
            }
